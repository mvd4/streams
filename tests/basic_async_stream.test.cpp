...
namespace streams
{
  ...
  TEST_CASE( "basic_async_stream (lockfree queue)" )
  {
    struct test_observer
      : basic_observer< int, access_policy::none >
    {
    ...
    basic_async_stream< int, access_policy::none, lockfree_queue_t > stream( 100u );
    ...
    SECTION( "All pushed events are removed from basic_stream after being dispatched once" )
    {
      std::vector< int > receivedValues;
      test_observer o;
      stream.subscribe( o );
  ...
  TEST_CASE( "basic_async_stream (std::deque)" )
  {
    struct test_observer
      : basic_observer< int, access_policy::none >
    {
    ...
    basic_async_stream< int, access_policy::none > stream( default_queue< int >( 100u ) );
    
    SECTION( "Copying basic_async_stream copies the queue" )
    {
      std::vector< int > receivedValues;
    ...
    SECTION( "Moving basic_async_stream moves the queue" )
    {
      std::vector< int > receivedValues;
  ...
  TEST_CASE( "basic_async_observer (lockfree queue)" )
  {
    ...
    basic_stream< int, access_policy::none > stream;
